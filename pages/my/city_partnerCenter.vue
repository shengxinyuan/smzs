<template>
	<view>
		...
		<view class="cont">
			<view class="cont_item" v-for="(it,ind) in 14" :key="ind">
...
<script>
	export default{
		...
		computed:{
			//提成
			mon_pic(){
				let arr = this.month_price.toFixed(2)
			...
			//本月业绩
			thi_mon(){
				return this.thismonth && this.thismonth.toString().replace(/\d+/, (s)=>{
					return s.replace(/(\d)(?=(\d{3})+$)/g, '$1,')
				})
			},
			// 列表数据
			list_price(){
				return this.list_pic && this.list_pic.toString().replace(/\d+/, (s)=>{
					return s.replace(/(\d)(?=(\d{3})+$)/g, '$1,')
				})
			}
		},
	...
	.header{
		...
		.data_statis{
			...
			.data_bottom{
				...
				padding-top: 6rpx;margin-top: 20rpx;
				view{
					width: 33%;text-align: center;height: 46rpx;border-right: 2rpx solid #666;line-height: 56rpx;color: #666;font-size: 26rpx;
				}
				view:last-child{
					border: 0;
				}
