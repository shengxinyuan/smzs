<template>
	<view class="container">
		<zs-hx-navbar ref="hxnb" :config="config">
		...
		<view class="trabecula" v-if="detail_type == 2">
			<text class="trabecula_l">限时秒杀中</text>
			<view style="margin-left: 20rpx;"> 距结束 <u-count-down :timestamp="second" color="white" bg-color="#E8372F">
				</u-count-down>
			</view>
		</view>
		...
		<view class="shop_names" v-if="detail_type == 2">
		...
		<view class="group_sty" v-if="detail_type == 1 && pi_group.length > 0">
		...
		<view class="add_bottom">
			<view class="add_con">
				...
				<view class="anniu" v-if="detail_type == 2">
		...
		<u-popup v-model="skuShow" mode="bottom">
			<scroll-view scroll-y="true" >
				<view class="attr-content" @click.stop="stopPrevent" style="height: 900rpx;position: relative;">
					...
					<view v-for="(it, ind) in details" :key="ind" 
					:class="{jactive:jg_ind == it.id}" class="jg_sty"
						@click="zhifu(it)">
						<view class="jg_sty_t">
							<view>
								条码：{{it.bar_code}}
					...
					<view style="position: fixed;bottom: 0;left: 0;right: 0;padding: 0 20rpx;background-color: #FFFFFF;">
						<view style="margin: 20rpx 0;display: flex;align-items: center;justify-content: space-between;">
							...
							<u-number-box v-model="value" :min="1" :step="1" :max="stocks" :long-press="false" @change="valChange">
							</u-number-box>
						</view>
