	...
	<view class="hx-navbar" :style="{width:screenWidth + 'px'}">
		<view class="hx-navbar__content"
			...
			<view class="hx-navbar__content__imgctn" :style="{'opacity': bgImgTransparent[1]}" v-if="backgroundImage">
				<image class="hx-navbar__imgctn__img"
					:style="{'height': conf.height + statusBarHeight + 'px',width:screenWidth + 'px'}"
					:src="backgroundImageEnd" mode="scaleToFill"></image>
			</view>
			<view class="hx-navbar__content__imgctn" :style="{'opacity': bgImgTransparent[0]}" v-if="backgroundImage">
				<image class="hx-navbar__imgctn__img"
					:style="{'height': conf.height + statusBarHeight + 'px',width:screenWidth + 'px'}"
					:src="backgroundImage" mode="scaleToFill"></image>
			</view>
			<!-- #ifndef H5 -->
			<!-- 状态栏 -->
			<view
				:style="'height:' + statusBarHeight + 'px;' + (statusBarBackground ? 'background-color:'+ statusBarBackground : '')"
				class="hx-navbar__status" v-if="conf.statusBar"></view>
			<!-- #endif -->
			<!-- 小程序分开编译 -->
			<!-- #ifdef MP -->
			<view class="hx-navbar__content__main"
				...
				<view class="hx-navbar__content__main" :style="{ height: conf.height + 'px', fontSize: conf.fontSize}">
					...
					<view class="hx-navbar__content__main_back" @tap="onBack" v-if="conf.back">
						<text class="hxicon hx-navbar__icon" :class="{'hx-navbar__icontran':bgTransparent == 0}"
							:style="{color: txtColor}">&#xe679;</text>
						<text class="hx_font_size" :style="{color: txtColor}"
							v-if="conf.backTxt">{{conf.backTxt}}</text>
					</view>
					<view class="hx-navbar__content__main_left" style="padding: 0;margin:0 13px 0 0;"
						v-if="conf.leftSlot">
						<slot name="leftSwitch" v-if="conf.leftSlotSwitch && slotSwitchOpacity == 0" />
						<slot name="left" v-else></slot>
					</view>
					<view class="hx-navbar__content__main_left"
						:style="{'padding': conf.back ||conf.leftSlot ? '0' : '0 9px 0 13px;' }" v-if="conf.leftButton">
						<block v-for="(btn,index) in conf.leftButton" :key="index">
							<view class="hx-navbar__content__main_left_btn" @tap="onClickBtn(btn)"
								:class="{'hx-navbar__btntran':bgTransparent == 0}">
								<text v-if="btn.position && btn.position == 'left'" class="hx-navbar__icon"
									:class="conf.font"
									:style="{color:btn.color ? btn.color : txtColor,marginRight:btn.txt ? '2px': '0'}">{{iconHandle(btn.icon)}}</text>
								<text class="hx-navbar__content__main_lefticon_txt hx_font_size"
									:style="{color:btn.color ? btn.color : txtColor}" v-if="btn.txt">{{btn.txt}}</text>
								<text v-if="!btn.position || btn.position != 'left'" class="hx-navbar__icon"
									:class="conf.font"
									:style="{color:btn.color ? btn.color : txtColor,marginLeft:btn.txt ? '2px': '0'}">{{iconHandle(btn.icon)}}</text>
							</view>
						</block>
					</view>
					<!-- #endif -->
					...
					<view class="hx-navbar__content__main_center" v-if="!conf.search && !conf.maxSlot">
						...
						<view class="hx-navbar__content__main_center_txt">
							<text class="hx_font_size hx_text_overflow" :style="{color: txtColor}"
								v-if="!conf.centerSlot && title">{{title}}</text>
					...
					<view class="hx-navbar__content__main_center" style="overflow: hidden"
						v-if="!conf.search && conf.maxSlot">
					...
					<view class="hx-navbar__content__main_left"
						...
						<block v-for="(btn,index) in conf.leftButton" :key="index">
							<view class="hx-navbar__content__main_left_btn" @tap="onClickBtn(btn)"
								...
								<text v-if="btn.position && btn.position == 'left'" class="hx-navbar__icon"
									:class="conf.font"
									:style="{color:btn.color ? btn.color : '',marginRight:btn.txt ? '2px': '0'}">{{iconHandle(btn.icon)}}</text>
								<text class="hx-navbar__content__main_lefticon_txt hx_font_size"
									:style="{color:btn.color ? btn.color : txtColor}" v-if="btn.txt">{{btn.txt}}</text>
								<text v-if="!btn.position || btn.position != 'left'" class="hx-navbar__icon"
									:class="conf.font"
									:style="{color:btn.color ? btn.color : '',marginLeft:btn.txt ? '2px': '0'}">{{iconHandle(btn.icon)}}</text>
							</view>
						</block>
					</view>
					<!-- #endif -->

					<view class="hx-navbar__content__main_search"
						:style="{'padding-left': conf.leftButton ? '9px' : '13px','padding-right': conf.rightButton ? '9px' : '13px'}"
						v-if="conf.search" @click="searchClick">
						<text class="hxicon hx-navbar__content__main_search_hxicon"
							style="color:#666;">&#xe65c;</text>
						<input type="text" class="hx-navbar__content__main_search_input hx_font_size"
							placeholder-style="color:#666;" confirm-type="search" :value="conf.search.value"
							:placeholder="conf.search.placeholder ? conf.search.placeholder : '请输入搜索内容'"
							:disabled="conf.search.disabled" @confirm="searchConfirm" 
							:style="{'background-color':conf.search.bgColor}" />
					</view>
					...
					<view class="hx-navbar__content__main_right"
						...
						<block v-for="(btn,index) in conf.rightButton" :key="index">
							<view class="hx-navbar__content__main_right_btn" @tap="onClickBtn(btn)"
								:class="{'hx-navbar__btntran':bgTransparent == 0}">
								<text v-if="btn.position && btn.position == 'left'"
									class="hxicon hx-navbar__content__main_right_icon hx-navbar__icon"
									:class="conf.font"
									:style="{color:btn.color ? btn.color : '',marginRight:btn.txt ? '2px': '0'}">{{iconHandle(btn.icon)}}</text>
								<text class="hx-navbar__content__main_right_txt hx_font_size"
									:style="{color:btn.color ? btn.color : txtColor}" v-if="btn.txt">{{btn.txt}}</text>
								<text v-if="!btn.position || btn.position != 'left'"
									class="hxicon hx-navbar__content__main_right_icon hx-navbar__icon"
									:class="conf.font"
